    ...
    <div class="page-content">
        <div class="content-card">
            ...
            <div class="card-header-custom">
                <h3 class="card-header-title">Edit Question</h3>
                ...
                <form id="questionForm" novalidate>
                    <div class="row g-4">
                        ...
                        <div class="col-md-6">
                            ...
                            <select class="form-select form-control-custom" id="religion_id" name="religion_id" required>
                                ...
                                @foreach ($religions as $religion)
                                    <option value="{{ $religion->id }}" {{ $question->religion_id == $religion->id ? 'selected' : '' }}>
                                        {{ $religion->name }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-md-6">
                            ...
                            <select class="form-select form-control-custom" id="time_slot_id" name="time_slot_id" required>
                                ...
                                @foreach ($timeSlots as $timeSlot)
                                    <option value="{{ $timeSlot->id }}" {{ $question->time_slot_id == $timeSlot->id ? 'selected' : '' }}>
                                        {{ $timeSlot->name ?? ($timeSlot->title ?? 'Time Slot #' . $timeSlot->id) }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="col-12">
                            ...
                            <textarea class="form-control form-control-custom" id="question" name="question" rows="4" placeholder="Enter question here..." required>{{ $question->question }}</textarea>
                            <div class="invalid-feedback">Please enter a question.</div>
                        </div>

                        <div class="col-12">
                            <div class="form-check">
                                <input class="form-check-input" type="checkbox" id="status" name="status" value="1" {{ $question->status ? 'checked' : '' }}>
...
@push('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            const updateUrl = @json(route('questions.update', $question->id));
            ...
            form.addEventListener('submit', function(e) {
                ...
                submitBtn.innerHTML =
                    '<span class="spinner-border spinner-border-sm me-1 text-white" style="color: white;"></span> <span class="text-white">Updating...</span>';
